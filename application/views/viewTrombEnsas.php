...
//le w et le h de tte la page pdf
	...
	$pdf->Cell(200,7,'des �l�ves issus de la 2�me ann�e cycle pr�pa de l\'ENSAS',0,1,'C');
	//verifier la date
	...
	$pdf->SetFont('Arial','',12);
	$pdf->Cell(20,7,' El�ves .',0,0,'R');
	$pdf->Cell(20,7,' Fili�re :',0,0,'R');
	$pdf->SetFont('Arial','B',12);
	
	//changer le titre selon la filiere choisie
	if($results[0]->final_filiere=='F')
	{
	$pdf->Cell(50,7,'G�nie Informatique',0,1,'L');
	}
	if($results[0]->final_filiere=='D')
	{
	$pdf->Cell(50,7,'G�nie Industriel',0,1,'L');
	}
	if($results[0]->final_filiere=='P')
	{
	$pdf->Cell(50,7,'G�nie des Proc�d�s et M.C',0,1,'L');
	}
	if($results[0]->final_filiere=='T')
	{
	$pdf->Cell(50,7,'G�nie T�l�com et R�seaux',0,1,'L');
	}
    $pdf->Ln();
	$pdf->Ln();
	$pdf->Ln();
	...
	$xrect=4;
	$yrect=50;
	//� ne pas changer ds la boucle pr garder  ls m�mes valeurs pr ts les rect
	$wrect=39;
	$hrect=55;
